...
export function getLegalMovesP(position, board, enPsassantSquare) {
    ...
    if (board[position][0] === 'white-pawn') {
        ...
        if (positionToColumn(position) !== 7 && board[position - 7][0] !== null && (board[position - 7][0]?.includes('black') || (board[position - 7][0] === 'en-passant-square' && positionToRow(position)) === 3)) isLegal[position - 7] = true;
        if (positionToColumn(position) !== 0 && board[position - 9][0] !== null && (board[position - 9][0]?.includes('black') || (board[position - 9][0] === 'en-passant-square' && positionToRow(position)) === 3)) isLegal[position - 9] = true;
    }
        ...
        // if en passant is possible, add it to legal moves //
        if (positionToColumn(position) !== 0 && board[position + 7][0] !== null && (board[position + 7][0]?.includes('white') || (board[position + 7][0] === 'en-passant-square' && positionToRow(position)) === 4)) isLegal[position + 7] = true;
        if (positionToColumn(position) !== 7 && board[position + 9][0] !== null && (board[position + 9][0]?.includes('white') || (board[position + 9][0] === 'en-passant-square' && positionToRow(position)) === 4)) isLegal[position + 9] = true;
    }
